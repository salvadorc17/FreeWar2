...
namespace FactionsGame.Actors
{
    ...
    public class RTSUnit : Actor
    {
        ...
        public RTSUnit(FactionsGame game, string _name)
            : base(game, _name)
        {
            engine = (Engine)game;
            ...
            if (moveCommandIssued)
            {
                ...
                if (currentLocation != currentGoal)
                {
                    ...
                    if (!moveStepComplete)
                    {
                        ...
                        if (moveDisplacement.X >= engine.TileHeight || moveDisplacement.Y >= engine.TileHeight)
                        {
                    ...
                    else
                    {
                        if (pathStack.Count > 0)
                        {
                            ...
                            // Check solidity
                            Tile pathNodeTile = engine.TileExistenceCheckByGridRef(new GridReference(pathNode.Position.X, pathNode.Position.Y));
        ...
        public void MoveToGridLocation(Vector2 target)
        {
            ...
            AStarPathNode path = new AStarPathNode();  // currentLocation
            while (path != null)
            {
                pathStack.Push(path);
                //Tile thisTile = engine.TileExistenceCheckByGridRef((int)path.Position.X, (int)path.Position.Y);
