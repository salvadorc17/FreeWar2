...
namespace FactionsGame
{
    ...
    public class FactionsGame : Engine
    {
        ...
        protected override void LoadContent()
        {
            ...
            try
            {
                string soundsDir = FileAccess.GetSoundsDir();
                _audioEngine = new AudioEngine(Path.Combine(soundsDir, @"FactionsAudioXACT.xgs"));
                _soundBank = new SoundBank(_audioEngine, Path.Combine(soundsDir, @"Sound Bank.xsb"));
                _waveBank = new WaveBank(_audioEngine, Path.Combine(soundsDir, @"Wave Bank.xwb"));

                _soundEffectsCategory = _audioEngine.GetCategory("Default");
                _soundEffectsCategory.SetVolume(0.5f);
            }
            catch (Exception ex)
            {
                Log.Message("Audio engine failed to load: " + ex.Message);
            }


            PlaySound("CinematicBoom");
        }
        #endregion
